...
TEST_F(LoopbackStreamTest, Basic) {
  ...
  std::shared_ptr<libp2p::connection::Stream> stream =
      std::make_shared<LoopbackStream>(PeerInfo{peer_id, {}});
  stream->write(kBuffer, kBuffer.size(), [stream, buf = kBuffer](auto result) {
    ...
    stream->read(*read_buf, kBufferSize,
                 [buf = read_buf, source_buf = buf](auto result) {
                   EXPECT_OUTCOME_TRUE(bytes, result);
                   ASSERT_EQ(bytes, kBufferSize);
