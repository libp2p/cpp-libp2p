...
TEST_F(DialerTest, DialAllTheAddresses) {
  ...
  EXPECT_CALL(
      *transport,
      ...
      .WillOnce(Arg2CallbackWithArg(Q_ERROR(std::errc::connection_refused)));
...
TEST_F(DialerTest, NewStreamFailed) {
  ...
  EXPECT_CALL(*connection, newStream())
      .WillOnce(Return(Q_ERROR(std::errc::io_error)));
...
TEST_F(DialerTest, NewStreamNegotiationFailed) {
  ...
  EXPECT_CALL(*proto_muxer, selectOneOf(Truly(if_protocols), _, _, _, _))
      .WillOnce(InvokeArgument<4>(Q_ERROR(r)));
