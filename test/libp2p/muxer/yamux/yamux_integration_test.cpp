...
class YamuxIntegrationTest : public testing::Test {
 public:
  ...
  void withStream(std::shared_ptr<ReadWriteCloser> conn,
    ...
    auto new_stream_msg =
        std::make_shared<ByteArray>(newStreamMsg(expected_stream_id));
    auto rcvd_msg = std::make_shared<ByteArray>(new_stream_msg->size(), 0);
...
TEST_F(YamuxIntegrationTest, StreamFromClient) {
  ...
  auto new_stream_ack_msg_rcv =
      std::make_shared<ByteArray>(YamuxFrame::kHeaderLength, 0);
                  ...
                  *new_stream_ack_msg_rcv, YamuxFrame::kHeaderLength,
                  [this, created_stream_id, new_stream_ack_msg_rcv,
                   conn](auto &&res) {
                    ...
                    // stream
                    auto parsed_ack_opt = parseFrame(*new_stream_ack_msg_rcv);
...
TEST_F(YamuxIntegrationTest, Ping) {
  ...
  auto received_ping = std::make_shared<ByteArray>(ping_out_msg.size(), 0);
...
TEST_F(YamuxIntegrationTest, Reset) {
  ...
  auto rcvd_msg = std::make_shared<ByteArray>(expected_reset_msg.size(), 0);
