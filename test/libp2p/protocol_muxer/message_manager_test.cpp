...
class MessageManagerTest : public ::testing::Test {
 public:
  ...
  const std::vector<Protocol> kDefaultProtocols{
      "/plaintext/1.0.0", "/ipfs-dht/0.2.3", "/http/w3id.org/http/1.1.0"};
  static constexpr uint64_t kProtocolsListBytesSize = 63;
  ...
  const ByteArray kProtocolsMsg = [this]() -> ByteArray {
    ByteArray buffer = UVarint{kProtocolsListBytesSize}.toVector();
...
TEST_F(MessageManagerTest, ParseProtocols) {
  auto msg = gsl::make_span(kProtocolsMsg);
  EXPECT_OUTCOME_TRUE(parsed_protocols,
                      MessageManager::parseProtocols(msg.subspan(1)))
...
TEST_F(MessageManagerTest, ParseProtocol) {
  ...
  ASSERT_EQ(parsed_protocol, kDefaultProtocols[0]);
}
...
TEST_F(MessageManagerTest, ParseOpening) {
  ...
  ASSERT_EQ(parsed_protocol,
            kMultiselectHeaderProtocol.substr(
                0, kMultiselectHeaderProtocol.size() - 1));
}
