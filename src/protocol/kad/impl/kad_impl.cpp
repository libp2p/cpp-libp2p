...
#include <libp2p/protocol/kad/impl/kad_server.hpp>

OUTCOME_CPP_DEFINE_CATEGORY(libp2p::protocol::kad, Error, e) {
  using E = libp2p::protocol::kad::Error;
  switch (e) {
    case E::SUCCESS:
      return "success";
    case E::NO_PEERS:
...
namespace libp2p::protocol::kad {
  ...
  KadImpl::KadImpl(
      std::shared_ptr<Host> host, std::shared_ptr<Scheduler> scheduler,
  ...
  void KadImpl::findRandomPeer() {
    ...
    Kad::FindPeerQueryResultFunc handler = [this](const peer::PeerId &peer,
                                                  FindPeerQueryResult result) {
      if (result.success) {
    ...
    findPeer(peer_id, handler);
  }

  void KadImpl::randomWalk() {
    BOOST_ASSERT(config_.randomWalk.enabled);
