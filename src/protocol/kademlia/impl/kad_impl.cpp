...
namespace libp2p::protocol::kademlia {
  ...
  KadImpl::KadImpl(Host& host, std::shared_ptr<RoutingTable> table,
  ...
  void KadImpl::start(bool start_server) {
    if (start_server) {
      host_.setProtocolHandler(
        getProtocolId(),
        [wptr=weak_from_this()](protocol::BaseProtocol::StreamResult rstream) {
          auto h = wptr.lock();
          if (h) {
  ...
  const KademliaConfig& KadImpl::config() {
    return config_;
  }
  ...
  bool KadImpl::onFindNode(Message& msg) {
    ...
    if (id_res) {
      auto ids = table_->getNearestPeers(NodeId(id_res.value()), 20);
      ...
      for (const auto& p : ids) {
        ...
        auto connectedness = host_.getNetwork().getConnectionManager().connectedness(info);
  ...
  std::shared_ptr<Kad> createDefaultKadImpl(const std::shared_ptr<libp2p::Host>& h, std::shared_ptr<RoutingTable> rt) {
    return std::make_shared<KadImpl>(*h, std::move(rt));
  }
