...
namespace libp2p::connection {
  ...
  void YamuxStream::endRead(outcome::result<size_t> result) {
    ...
    // N.B. reentrancy of read_cb_{read} is allowed TODO(107)
    is_reading_ = false;
    bytes_waiting_ = 0;
    reading_some_ = false;
    if (read_cb_) {
  ...
  void YamuxStream::sendAck(size_t bytes) {
    ...
    yamuxed_connection_->streamAckBytes(
        stream_id_, bytes,
  ...
  void YamuxStream::read(gsl::span<uint8_t> out, size_t bytes,
    ...
    if (!cb || bytes == 0 || out.empty()
      ...
      return deferReadCallback(Error::INVALID_ARGUMENT, std::move(cb));
    }

    if (is_reading_) {
      return deferReadCallback(Error::IS_READING, std::move(cb));
    }

    auto res = tryConsumeReadBuffer(out, bytes, some);
    if (!res || res.value() > 0) {
      return deferReadCallback(res, std::move(cb));
    }

    // is_readable_ flag is set due to FIN flag from the other side.
    // Nevertheless, unconsumed data may exist at the moment
    if (!is_readable_) {
      return deferReadCallback(Error::NOT_READABLE, std::move(cb));
    }
  ...
  void YamuxStream::write(gsl::span<const uint8_t> in, size_t bytes,
    ...
    if (!is_writable_) {
      return deferWriteCallback(Error::NOT_WRITABLE, std::move(cb));
    }
    ...
    if (is_writing_) {
      ...
      write_queue_.emplace_back(in_vector, bytes, std::move(cb), some);
      return;
    }
  ...
  void YamuxStream::adjustWindowSize(uint32_t new_size,
    ...
    if (is_writing_) {
      return deferWriteCallback(Error::IS_WRITING, [cb = std::move(cb)](auto) {
        cb(Error::IS_WRITING);
      });
    }
    ...
    if (new_size > maximum_window_size_ || new_size < read_buffer_.size()) {
      return deferWriteCallback(
          ...
          [cb = std::move(cb)](auto) { cb(Error::INVALID_WINDOW_SIZE); });
    }
