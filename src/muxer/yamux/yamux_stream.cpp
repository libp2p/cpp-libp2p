...
namespace libp2p::connection {
  ...
  void YamuxStream::read(MutSpanOfBytes out, size_t bytes,
                         ReadCallbackFunc cb) {
    doRead(out, bytes, std::move(cb), false);
  }

  void YamuxStream::readSome(MutSpanOfBytes out, size_t bytes,
                             ReadCallbackFunc cb) {
  ...
  void YamuxStream::write(ConstSpanOfBytes in, size_t bytes,
                          WriteCallbackFunc cb) {
    doWrite(in, bytes, std::move(cb), false);
  }

  void YamuxStream::writeSome(ConstSpanOfBytes in, size_t bytes,
                              WriteCallbackFunc cb) {
  ...
  void YamuxStream::doRead(MutSpanOfBytes out, size_t bytes,
  ...
  void YamuxStream::doWrite(ConstSpanOfBytes in, size_t bytes,
